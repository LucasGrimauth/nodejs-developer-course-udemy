...
describe("DELETE /todos/:id", () => {
  ...
  it("Shouldn't remove a todo from other user", done => {
    const hexId = todos[1]._id.toHexString();
    request(app)
      .delete(`/todos/${hexId}`)
      .set("x-auth", users[0].tokens[0].token)
...
describe("PATCH /todos/:id", () => {
  ...
  it("Shouldn't update the todo of another user", done => {
    const hexId = todos[0]._id.toHexString();
    var text = "This should be the new text";
...
describe("POST /users", () => {
  ...
  it("Shouldn't create user if email is in use", done => {
    request(app)
      .post("/users")
      .send({ 
        email: users[0].email,
